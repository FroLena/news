...
async def rewrite_news(text):
    ...
    system_prompt = (
        ...
        f"   - Опрос (Если тема резонансная).\n\n"
        
        f"=== ЧАСТЬ 3. ПРАВИЛА КАРТИНКИ (DIGITAL STOCK PHOTO) ===\n"
        ...
        f"=== ШАБЛОН ОТВЕТА (СТРОГО СОБЛЮДАЙ) ===\n"
        f"||R:🔥|| <b>Заголовок новости</b>\n"
        f"\n"
        f"Текст новости одним абзацем без воды.\n"
        ...
        f"Ответ 2\n"
        f"|||\n"
        f"Commercial photo of [Scene description]. Bright lighting, sharp focus, 8k."
    )
