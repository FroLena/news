...
# --- AI РЕДАКТОР (СТРОГИЙ ПРИОРИТЕТ) ---
async def rewrite_news(text, history_topics):
    history_str = "\n".join([f"- {t}" for t in history_topics[-15:]]) if history_topics else "Нет истории."

    system_prompt = (
        f"Ты — главный редактор новостного канала. Твоя задача — фильтрация и рерайт.\n\n"
        f"ИСТОРИЯ ПОСЛЕДНИХ СОБЫТИЙ (Уже опубликовано):\n{history_str}\n\n"
        f"АЛГОРИТМ ДЕЙСТВИЙ (СТРОГО ПО ПОРЯДКУ):\n"
        f"1. СРАВНИ С ИСТОРИЕЙ. Если новость об том же событии, что и в списке выше (даже если добавлены мелкие детали) -> ВЕРНИ СЛОВО: DUPLICATE\n"
        f"2. ПРОВЕРЬ НА МУСОР. Если это реклама, просьба подписаться, казино или просто не новость -> ВЕРНИ СЛОВО: SKIP\n"
        f"3. ЕСЛИ ЭТО НОВАЯ НОВОСТЬ -> Делай рерайт.\n\n"
        f"ПРАВИЛА РЕРАЙТА (Russian HTML):\n"
        f"- Перепиши новость полностью своими словами (Инфостиль).\n"
        f"- Заголовок: короткий, жирный, без слова 'Заголовок'.\n"
        f"- В конце: <blockquote><b>📌 Суть:</b> [Вывод]</blockquote>\n"
        f"- В начале: реакция ||R:🔥||\n"
        f"- Если нужно: ||POLL||\n\n"
        f"ФОРМАТ ОТВЕТА: ТЕКСТ ||| ПРОМПТ_КАРТИНКИ\n"
        f"(Промпт картинки на английском: Hyperrealistic documentary photo, 8k, no text)."
    )
...
if __name__ == '__main__':
    ...
    scheduler.start()
    print("🤖 Бот запущен! (Priority: 1.Duplicate 2.Trash)")
