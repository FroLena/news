...
import sqlite3
import pytz
import difflib # Библиотека для умного сравнения текста
...
# --- AI РЕДАКТОР (ТВОЙ ПРАВИЛЬНЫЙ ПРОМПТ) ---
async def rewrite_news(text):
    history_items = load_history()
    recent_history = history_items[-30:] # Увеличили контекст
...
async def handler(event):
    ...
    # --- УМНЫЙ ФИЛЬТР ДУБЛЕЙ (Difflib) ---
    # Сравниваем новый текст со списком последних 50 текстов
    is_duplicate = False
    for old_text in raw_text_cache:
        # Сравниваем похожесть строк (0.0 - 1.0)
        matcher = difflib.SequenceMatcher(None, text, old_text)
        if matcher.ratio() > 0.65: # Если совпадение > 65% - удаляем
            is_duplicate = True
            break
    
    if is_duplicate:
        print(f"♻️ Сработал Fuzzy-фильтр (сходство > 65%)")
        stats_db.increment('rejected_dups')
        return

    # Добавляем в кэш (храним 50 последних текстов)
    raw_text_cache.append(text)
    if len(raw_text_cache) > 50: raw_text_cache.pop(0)
    ...
    try:
        ...
        if sent_msg:
            ...
            if "<blockquote>" in news_text:
                try: 
                    # Берем то, что внутри blockquote
...
if __name__ == '__main__':
    ...
    if client:
        ...
        print("🤖 Бот запущен! (FULL VERSION: FUZZY + STATS + PODCAST)")
